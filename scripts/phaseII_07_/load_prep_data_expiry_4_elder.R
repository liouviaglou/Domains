# Rscript --vanilla load_prep_data_expiry_4_elder.R myriad-303821 expiry 2021-01-04 2021-06-30


# Old:
# Rscript --vanilla load_prep_data_expiry_4_elder.R myriad-303821 expiry 2021-02-23
...
# query_str <- gsub("[\r\n]", " ", paste0("SELECT * FROM  prediction_vendors.base_expiry_dp_data t WHERE
#   DATE(expiry_date) BETWEEN \"",mindate,"\" AND \"",maxdate,"\" AND renewed_count=1 AND  
#   ABS(HASH(expiry_date)) % 100 < 10"))

# command_str <- paste0("bq query --max_rows=0 --project_id='radixbi-249015' --destination_table='", tblloc_str_2,"' '", query_str ,"'")
...
download_from_radix_bq <- function(tbl_suffix, mindate, maxdate, bucket,
    ...
    if (bypass_bq_download) {  # bq_table_download has trouble with rate limits for me
        ...
        # Cloud to Local
        system(paste0("gsutil cp -r ",dest," ", directory))
...
# if (bypass_bq_download) {  # bq_table_download has trouble with rate limits for me
#     # BQ to Cloud
#     dest <- paste0(bucket, tblname_str_2, ".csv")
#     bq_table_save(bq_project_query(projname_str, paste0("SELECT * FROM  ",dbname_str,'.',tblname_str_2)
#                                                 ), destination_uris = dest, destination_format = "CSV")
...
#     # Read in from Local
#     eval(call("<-", as.name(tblname_str_2), 
#               read.csv(paste0(directory, "/", tblname_str_2, ".csv"))))
# } else {
