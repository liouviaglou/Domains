# Rscript ranger_hyperparam.R >> /home/jupyter/local/Domains_202003/data/output/hyper_grid_std_weights.log 2>&1
...
# # load completed grid
fname = "/home/jupyter/local/Domains_202003/data/output/hyper_grid_weights.log"
...
names(hyper_grid_todo) <- c("w1","w2","OOB_RMSE","timestamp")
...
for(i in 1:nrow(hyper_grid_todo)) {
  ...
  cat(paste(ts))
  cat(paste("\n"))
  cat(paste(sprintf("Processing w1=%f, w2=%f; \n", w1,w2)))
