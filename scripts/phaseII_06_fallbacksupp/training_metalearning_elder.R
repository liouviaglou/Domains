# Rscript training_metalearning_elder.R > /home/jupyter/Domains_202003/data/output/training_metalearning7.log 2>&1
...
source('functions_fallback_elder.R')
source('functions_metalearning.R')
source('../phaseII_03_forest/functions_eval.R')


dataDir='/home/jupyter/Domains_202003/data/output/datapull_20210223'
modelDir='/home/jupyter/Domains_202003/data/output/models_20201104'
outputDir='/home/jupyter/Domains_202003/data/output/datapull_20210223'

today <- as.Date("2021-02-23")
maxdate <- today - 50
mindate <- maxdate - 456

train_str <- paste0('expiry_',format(mindate, format="%Y%m%d"),'_',format(maxdate, format="%Y%m%d") ,'_train1')
valid_str <- paste0('expiry_',format(mindate, format="%Y%m%d"),'_',format(maxdate, format="%Y%m%d") ,'_train2')
test_str <- paste0('expiry_',format(mindate, format="%Y%m%d"),'_',format(maxdate, format="%Y%m%d") ,'_test')
...
cat("Reading in csvs..\n\n")
expiry_df_test_preds <- read.csv(file.path(modelDir, "preds", "preds.csv"))

# Load training data used for predictions_metalearning.R to assign fallback values
expiry_df_train <- read.csv(file.path(dataDir, paste0(train_str, ".csv")))
...
expiry_new_df <- read.csv(file.path(dataDir, paste0(test_str, ".csv")))
...
tld_registrar_excl_list = tld_registrar_excl_df(train_df = expiry_df_train) # trained on old data, exclusion based on old data
...
########################################################################################################
#
# SUPPLEMENT validation predictions w/ FALLBACK
#
########################################################################################################                                             
                                               
# generate placeholder (*_fb_) columns in preds df where predictions for low-volume tld-registrars get set to NA
preds_df <- preds_df %>%
     mutate( across(contains('pred_'), 
                    .fns = list(fb_ = ~ifelse(tld_registrar_index %in% tld_registrar_excl_list, NA, . )) ))

# apply fallback tables (creating cols *_fb)
preds_df <- fallback_app_1(test_data_op=preds_df,
               in_col='pred_seg2_rf_ALL_fb_',
               out_col='pred_seg2_rf_ALL_fb')
preds_df <- fallback_app_1(test_data_op=preds_df,
               in_col='pred_seg2_glm_ALL_fb_',
               out_col='pred_seg2_glm_ALL_fb')
preds_df <- fallback_app_1(test_data_op=preds_df,
               in_col='pred_seg_rf_ALL_fb_',
               out_col='pred_seg_rf_ALL_fb')
preds_df <- fallback_app_1(test_data_op=preds_df,
               in_col='pred_seg_glm_ALL_fb_',
               out_col='pred_seg_glm_ALL_fb')
preds_df <- fallback_app_1(test_data_op=preds_df,
               in_col='pred_agg_rf_fb_',
               out_col='pred_agg_rf_fb')
preds_df <- fallback_app_1(test_data_op=preds_df,
               in_col='pred_agg_glm_fb_',
               out_col='pred_agg_glm_fb')
preds_df <- fallback_app_1(test_data_op=preds_df,
               in_col='pred_agg_rf_ALL_fb_',
               out_col='pred_agg_rf_ALL_fb')
preds_df <- fallback_app_1(test_data_op=preds_df,
               in_col='pred_agg_glm_ALL_fb_',
               out_col='pred_agg_glm_ALL_fb')                                               

# Drop intermediate columns
preds_df <- preds_df[,-grep("fb_", names(preds_df))]
                                               
########################################################################################################
#
# Create pred_meta columns
#
########################################################################################################                                               
                                               
rel_meta_cols <- c('tld_registrar_index', 'l10_win_04_pred_model', 'auc_win_04_pred_model')
preds_df <- merge(preds_df, new_metametrics_imp_pred_df[, rel_meta_cols], on="tld_registrar_index", all.x = TRUE)                                               
                                                                                           
preds_df[,'pred_meta_l10'] <- NA
preds_df[, 'pred_meta_auc'] <- NA

models_to_column_names <- list(
    agg_rf_ALL = 'agg_rf_ALL',
    seg2_glm = 'seg2_glm_ALL',
    seg2_glm_fb = 'seg2_glm_ALL_fb'
)
for (model in unique(c(preds_df$l10_win_04_pred_model, preds_df$auc_win_04_pred_model))) {
    pred_col <- paste0('pred_', models_to_column_names[[model]])
    for (metric in c('l10', 'auc')) {
        model_col <- paste0(metric, '_win_04_pred_model')
        metric_col <- paste0('pred_meta_', metric)
        mask <- preds_df[, model_col] == model
        preds_df[mask, metric_col] <- preds_df[mask, pred_col]
    }
}                                               
...
############################################################################################
#
# Convert Wide to Long and write to BQ table
#
# #######################################################################################################                                  
                                               
# Convert wide to long
pred_vars <- names(preds_df)[grepl("^pred_", names(preds_df))]
preds_df_melt <- melt(setDT(preds_df), measure.vars = pred_vars, variable.name = "model", value.name = "predicted")
                                               
# Save to bq table in chunks
cat(paste0("Saving meta_preds with ", nrow(preds_df_melt), " rows\n"))
table_name <- "myriad-303821.expiry.expiry_meta_preds"
...
while (curr_end < nrows) {
    ...
    bq_table_upload(table_name, preds_df_melt[curr_start:curr_end, ], fields=preds_df_melt, 
